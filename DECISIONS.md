...
## Phase 6 — torch version
- The newest torch CPU wheel (2.12.1+cpu) imported once then failed with `WinError 1114` loading `c10.dll` (DLL init failed) on this Windows box. Pinned to **torch 2.3.1+cpu**, which loads reliably. requirements.txt documents the install command.

## Phase 4/5 — target & baseline notes
- Two targets live directly in the data: `label_binary` (correct/incorrect) and `label_exercise` (m01–m10). Mapping table exported to results/exercise_mapping.csv.
- Exercise 10-class is trivially separable (nearest-centroid baseline = 1.0) because the 10 exercises have very distinct joint-angle signatures — realistic and honest. The headline ML task is therefore **binary correctness (movement-quality assessment)**, where the rule baseline is weak (~0.57) and ML should add real value.
- Ablation: UI-PRMD angle data has no separate position channel, so the spec's "angle vs position" ablation is replaced by an angle-statistic-group ablation; "summary vs sequence" is covered by MLP-vs-CNN.
